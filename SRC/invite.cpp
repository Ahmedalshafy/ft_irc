...
void Server::handleInviteCommand(Client *client, const ParseMessage &ParsedMsg)
{
    ...
    if (params.size() < 2) {
        client->getServerReplies().push_back(ERR_NEEDMOREPARAMS(client->getNickname(), "INVITE"));
        return;
    }
    ...
    targetClient->getServerReplies().push_back(response);
    ...
    client->getServerReplies().push_back(inviteMessage);
}
